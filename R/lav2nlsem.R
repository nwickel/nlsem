lav2nlsem <- function(model, model.class=c("singleClass", "semm", "nsemm"),
                      constraints=c("indirect", "direct1", "direct2")
                      class.spec="class") {

  dat <- lavMatrixRepresentation(lavaanify(model, meanstructure=TRUE,
    auto=TRUE))
  ...
  num.classes <- length(unique(dat[, class.spec]))
  ...
  model.class <- match.arg(model.class)
