...
# last mod Sep/29/2015, NU

#--------------- main functions ---------------

phi_mix <- function(object) {
  ...
  if (object$model.class == "singleClass") {
  ...
  } else if (object$info$constraints != "indirect") {
    ...
    for (class in paste0("class", seq_len(object$info$num.classes))) {
      ...
      phi[(n+1):nrow(phi), seq_len(n)] <- cov_xyz(object)[[class]]
      phi[seq_len(n), (n+1):nrow(phi)] <- t(phi[(n+1):nrow(phi), seq_len(n)])
      tmp <- matrix(nrow=nl, ncol=nl)
      tmp[lower.tri(tmp, diag=T)] <- cov_xy(object)[[class]]
      phi[(n+1):nrow(phi), (n+1):nrow(phi)] <- fill_symmetric(tmp)
  ...
  ind.omega <- matrix(seq_len(object$info$num.xi*object$info$num.xi),
    object$info$num.xi, object$info$num.xi)
  ind.omega <- ind.omega[upper.tri(ind.omega, diag=TRUE)]
  ...
  for (class in names(pars)) {
    names.l <- names(pars[[class]])[grep("Gamma", names(pars[[class]]))]
    names.nl <- names(pars[[class]])[grep("Omega", names(pars[[class]]))]
...
standardize <- function(object) {
  ...
  # direct approaches (direct1 and direct2)
  if (object$info$constraints != "indirect") {
